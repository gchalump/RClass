...
for(i in 1:length(missing_percent)){
  ...
  df <- original
  is.na(df[sample(seq_len(nrow(df)), floor(nrow(df)*missing_percent[i]*0.8)),1:2]) <- TRUE
  if(floor(nrow(df)*missing_percent[i]*0.2) > 0){
    is.na(df[sample(seq_len(nrow(df)), floor(nrow(df)*missing_percent[i]*0.2)),3:4]) <- TRUE
  }
  summary(df)
...
ggplot(result_rmse, aes(percent)) +  
  ...
  labs(title = "RMSE from deliberately imputation methods",y = "RMSE", x = "Percentage of Missing", colour = "Methods")
...
ggplot(result_knn, aes(percent)) + 
  ...
  geom_line(aes(y = mean, colour = "mean"))+ 
  geom_line(aes(y = sample, colour = "sample"))+ 
  labs(title = "KNN acuuracy from deliberately imputation methods",y = "Accuracy", x = "Percentage of Missing", colour = "Methods")
